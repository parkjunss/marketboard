...
> 최종 갱신: 2026-09-10 (현재 소스 및 로컬 검증 기준)
...
## 2026-09-10 브라우저 검증 및 운영 적용 상태

- 운영 대상은 문서상 라즈베리파이의 `marketboard-mysql`이다. SSH 설정의 `raspberrypi`(jun@192.168.0.174)로 연결을 시도했지만 publickey/password 인증이 거절됐다. 운영 DB 조회·백업·마이그레이션은 수행하지 않았고, 접속 가능한 키 경로/계정 확인을 요청했다. 로컬 공유 MySQL을 운영 DB로 대신 처리하지 않았다.
- 별도 MySQL 8.4/Redis 및 backend 18080/frontend 13100에서 실제 Codex 브라우저를 사용했다. 테스트 계정·QAONLY 종목만 사용했으며 운영 보유 데이터는 변경하지 않았다.
- 확인: 로그인/로그아웃, 새로고침 시 갱신 세션 복구, 포트폴리오 생성·보유 추가, 오래된 버전 저장의 충돌 메시지, 최신 15주 재조회 후 20주 저장, 자료 누락을 포함한 점검 저장, 재접속 후 과거 점검 열기. 현재 보유를 25주로 바꿔도 저장 기록의 20주·버전 2가 유지됐다.
- 브라우저에서 발견한 로그인 이동 경합을 수정했다. RedirectIfAuthed/회원가입/관리자 권한 거절의 이동 대상을 `/review`로 통일하고 실제 재로그인 결과를 확인했다.
- 실시간 외부 시세 수집기는 연결하지 않았으므로 실제 시장 수치 정확성·정상 시세의 시각적 대조·운영 도메인 검증은 포함하지 않는다. 전용 검증 서버와 컨테이너는 종료/제거했다.
- 기본 원칙 보강과 브라우저 수정분을 로컬 커밋 대상으로 정리했다. 원격 push와 운영 배포는 수행하지 않았다.

## 2026-09-10 기본 원칙 보강 — 최신 작업

- 보유 수정: V24 `@Version` 및 필수 요청 버전, DB 동시 변경/오래된 편집 409. 포트폴리오 화면에 편집·충돌 안내·최신 조회 흐름을 연결했다.
- 생성 멱등성: V25에 사용자/작업/키/입력 해시/최초 응답을 저장한다. 포트폴리오 생성·백테스트 실행·점검 저장에 UUID 헤더를 요구하고 업무 변경과 결과 기록을 함께 커밋한다. 프론트는 불확실한 응답의 요청 키를 sessionStorage에 유지한다.
- 점검 근거: V26과 `/api/reviews` 생성/최근 50개 목록/소유자 상세 API, `/review` 저장/과거 기록 열기를 구현했다. 서버 원본 일봉·보유 가격/버전·평가 요약·실패 내역·수집 시각을 고정하며 최신 조회와 구분한다.
- 인증 연결 보완: 기존 보호 경로의 미인증 403을 JSON 401로 수정하여 토큰 갱신 경로와 일치시켰다. 관리자 권한 부족은 403을 유지한다.
- 검증: 백엔드 전체 70개 테스트 통과. 별도 MySQL 8.4에서 V24~V26 적용 성공 기록을 확인하고 동시 수정·멱등성·스냅샷 영속성 3개 및 HTTP 통합 1개 테스트를 통과했다. HTTP는 실제 서비스/DB와 MockMvc를 사용하며 외부 백테스트 수집기만 대역이다. 프론트 11개 테스트, 타입 검사·ESLint·프로덕션 빌드 통과. 실제 브라우저/운영 배포는 미검증이다.
- 운영 DB에는 마이그레이션을 적용하지 않았다. 배포 시 백엔드/프론트를 함께 갱신해야 한다(신규 요청 헤더·보유 버전 필수). 사용자별 보호된 생성은 DB 잠금으로 직렬화하므로 긴 백테스트가 같은 사용자의 다른 생성 요청을 지연시킬 수 있다.
- 후속: 백테스트 비동기 접수/중단 복구, 여러 탭 갱신 동기화, 전략 버전/판단 기록/알림. 현재 스냅샷은 전략 기반 ReviewRun 전체 구현이 아니다. 커밋/푸시는 아직 하지 않았다.

## 다음 세션 시작점

**2026-09-10 F03/F10 가격 자료 신뢰성 보강**:
- 이전 백테스트 수정·개발 기준 문서를 로컬 커밋 `5affd7f`로 저장했다. 원격 push는 하지 않았다.
- collector 시세에 원천·관측/수집 시각을 구분했다. REST 보충은 거래 시각을 모르면 null로 보내며 기존 틱 시각도 명시적으로 지운다.
- 포트폴리오 API에 가격 원천·관측 시각·수집 시각·일봉 날짜·자료 상태 및 평가 가능/누락/오래됨/미확인 종목 수를 추가했다. 단일/일괄 조회를 통일하고 10일 이전 일봉도 표시한다. 출처 없는 기존 자료는 최신성을 추정하지 않는다.
- 화면에서 '전일 종가'를 실제 일봉 날짜로, 부분 평가의 '총액'을 평가 대상 합계로 바꾸고 자료 경고와 초기 API 조회 실패를 표시한다.
- 검증: collector 전체 66개 통과, backend 전체 64개 통과(H2 + 임시 Redis 16379), 이후 추가한 가격 선택/JSON 검사를 포함해 관련 11개 재검증 통과. frontend 타입 검사·lint·production build 통과, 마지막 오류 상태 복구 변경 후 타입 검사·lint 재통과. 임시 Redis는 검증 후 종료·제거했다.
- 한계: 실제 MySQL·인증된 브라우저·배포 확인은 아직 하지 않았다. 거래소 달력 기반 최신 완료 세션, 기업행사/가격 보정, 재무·시장 지표의 범용 데이터 메타데이터는 후속이다. 기존 일봉은 날짜와 UNVERIFIED를 제공하며 A03 전체 완료로 취급하지 않는다.
...
# 2026-09-10 투자 점검 화면 구현

- 선행 데이터 신뢰성 보강을 `a484c6b`로 커밋했다.
- `/review`에 시장 환경 → 보유 자료 → 판단 준비 흐름을 구현하고 로그인/앱 첫 진입 및 사이드바에 연결했다. 기존 API의 6개 시장 지수, 시장 폭, 사용자 포트폴리오를 독립 조회한다.
- 주간 5개/월간 21개 관측 일봉 간격 비교와 실제 비교 날짜, 금리 bp 변화, 자료 부족/오류를 표시한다. 포트폴리오는 부분 평가·가격 누락·관측 품질과 통화/환산 기준 한계를 표시한다.
- 검증: 계산 회귀 테스트 3개, TypeScript, ESLint, Next.js 프로덕션 빌드 통과. 인증된 실제 브라우저, 실제 DB/API 응답 및 배포 검증은 미실시.
- 후속: 전략 버전·목표 비중 연결, ReviewRun 및 판단 기록 저장, 이메일/모바일 알림. 현재 화면은 F04/F07 조회 초안이며 자동 판단이나 주문을 생성하지 않는다.
# 2026-09-10 토큰 원자적 교체

- Redis 비교·교체·TTL을 Lua 한 번으로 실행하고 JWT jti를 추가했다. 토큰 갱신은 무조건 저장 경로를 사용하지 않는다.
- 프론트 동일 인증 인스턴스의 갱신 요청 병합, 늦은 401의 최신 토큰 재사용, 로그아웃 후 갱신 응답 무시를 구현했다. 재시도한 리소스 요청의 500 오류가 정상 갱신 세션을 지우지 않도록 분리했다.
- 백엔드 전체 66개 테스트 통과(실제 격리 Redis 경합 검사 포함). 프론트 8개 테스트 통과(인증 콜백 5개 + 기존 계산 3개), 타입 검사·빌드 통과. 실제 브라우저·배포는 미검증. 테스트 Redis는 종료/제거했다.
- 여러 탭/기기 조정, 기존 access token 즉시 폐기, 갱신 성공 응답 유실 복구는 후속 범위다.
