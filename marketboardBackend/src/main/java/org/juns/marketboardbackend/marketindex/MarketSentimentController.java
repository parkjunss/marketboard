...
/**
 * CNN Fear & Greed Index + SPY options put/call volume ratio. Fear & Greed still proxies the
 * collector directly (low request volume, and CNN's own data already only updates once or so a
 * day); put/call ratio is DB-backed on a schedule instead -- see PutCallRatioService for why. Both
 * sources are best-effort (CNN's undocumented internal endpoint, yfinance options data), so a
 * failure here is an expected "temporarily unavailable" case, not a bug.
 */
@RestController
